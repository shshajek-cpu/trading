...
데스크톱을 줄인 화면이 아니다. `MobileShell` 이 TradingView 앱처럼 아래 탭과 아래 시트로 그린다. 차트 영역은 데스크톱과 같은 레이아웃(1/2/4 분할, 설정 동기화로 공유)을 칸 경계 끌기 없이 똑같이 나눠 보인다 — 세로 화면의 2분할은 위아래, 가로 화면은 좌우, 4분할은 2×2. 칸을 누르거나 끌면 그 칸이 활성(파란 테두리)이 되고, 아래 도구 줄·시트는 활성 칸을 다룬다.
...
- **레이아웃 시트**: 단일/2분할/4분할, 이 칸 크게 보기(뒤로가기로 분할 복귀), 모든 칸에 같이 적용(심볼·차트 종류·십자선). 바꾸면 데스크톱 레이아웃도 같이 바뀐다.
