    ...
    <div className={`push-box ${push.state === 'on' ? 'on' : 'off'}`}>
      {push.state === 'on' ? (
        <>
          ...
          <p className="push-desc">서버가 1분마다 시세를 확인합니다. 폰이 잠겨 있어도 받습니다.</p>
          <button type="button" className="ghost-btn push-off" onClick={() => void push.disable()}>
            끄기
          </button>
        </>
      ) : (
        <>
          ...
          <p className="push-desc">켜두면 앱을 닫아도 서버가 대신 감시해 알려줍니다.</p>
          <button
            type="button"
            className="cta push-on"
